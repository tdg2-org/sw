...
/* RECEIVE FROM PL*/
		Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR) RxBufferPtr, 0x40, XAXIDMA_DEVICE_TO_DMA);
		...
		for (idx = 0; idx < 8; idx++) {
			...
			//xil_printf("Rx data %d = %d\r\n", idx, (unsigned int)RxPacket[idx]);
			//xil_printf("Rx data %d = %x\r\n", idx, RxPacket[idx]);
			xil_printf("Rx data %d = %x%x\r\n", idx, rxhi,rxlo);
		}
