...
From xilinx xqspipsu_generic_flash_polled_example

BSP -> Drivers -> psu_qspi -> xqspipsu_generic_flash_polled_example

requires specific build with DFX two RMs (exact partial bitstreams sizes):

	zub repo, git hash: 896365cf7fd06cc4badea7ef864fbe83a1856ff0
...
// removed all except my device - ZUB - ISSI - IS25WP256E
// FlashID=0x9D 0x70 0x19
const FlashInfo Flash_Config_Table[] = {
	/* ISSI */
	/*is25wp256d*/
	{
...
u8 ReadBuffer2[(PAGE_COUNT2 * MAX_PAGE_SIZE) + (DATA_OFFSET + DUMMY_SIZE) * 8] __attribute__ ((aligned(64)));
...
int QspiPsuPolledFlashExample(XQspiPsu *QspiPsuInstancePtr, UINTPTR BaseAddress)
{
	...
	XQspiPsu_SelectFlash(QspiPsuInstancePtr, XQSPIPSU_SELECT_FLASH_CS_LOWER, XQSPIPSU_SELECT_FLASH_BUS_LOWER);
...
//	     Count < PageSize;
//	     Count++, UniqueValue++) {
//		WriteBuffer[Count] = (u8)(UniqueValue + TestVal);
//	}
...
//	}
	...
	Status = FlashRead(QspiPsuInstancePtr, 0x010000, MaxData, ReadCmd,
			   CmdBfr, ReadBuffer);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}

	Status = FlashRead(QspiPsuInstancePtr, 0x040000, MaxData2, ReadCmd,
			   CmdBfr, ReadBuffer2);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
//	for (UniqueValue = UNIQUE_VALUE, Count = 0; Count < MaxData; Count++, UniqueValue++) {
//		if (ReadBuffer[Count] != (u8)(UniqueValue + TestVal)) {
//			return XST_FAILURE;
//		} 	
//	}

	for (Count = 60; Count < 72; Count++) {
		xil_printf("RX(%d) = 0x%02x\n\r",Count,ReadBuffer[Count]);
	}
	xil_printf("--\n\r");
